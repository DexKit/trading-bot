    ...
    if (isBuy) {
        try {
            ...
            if (Number(randomBuyAmount) > market.minBuyUnit) {
                const quote = await getSwapQuote({ sellAmount, sellToken, buyToken, takerAddress, intentOnFill: true }, market.chainId);
        ...
        try {
            ...
            const quote = await getSwapQuote({ buyAmount, sellToken, buyToken, takerAddress, intentOnFill: false }, market.chainId);
            ...
            if (BigNumber.from(quote.data.sellAmount).lt(market.baseTokenBalance.balance)) {
                ...
                const quote = await getSwapQuote({ buyAmount, sellToken, buyToken, takerAddress, intentOnFill: true }, market.chainId);
                if (!IS_SIMULATION) {
                    const { data, to, value, gas, gasPrice } = quote.data;
                    const tx = await signer.sendTransaction({ data, to, value, gasLimit: gas, gasPrice });
                    await tx.wait();
